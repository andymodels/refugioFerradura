...
- **Google Maps** (`maps.app.goo.gl`, busca `google.com/maps/search/...`): use para confirmar nome, nota, avaliações, endereço, telefone, faixa de preço quando a matéria for sobre um estabelecimento específico.
...
- **Sempre em português.**
- **Nunca use travessão ("—")** em nenhum texto do subtítulo ou dos parágrafos. Reescreva com vírgula, ponto ou frases separadas.
...
- Título: reflete o conteúdo real, sem clickbait, com ângulo específico (evite fórmulas genéricas tipo "Conheça os melhores X de Y").
...
Título corresponde ao conteúdo? Há ângulo editorial claro? Algum dado foi inventado ou não confirmado? Tem clichê ou repetição? Parece propaganda? Já existe matéria parecida publicada (busca feita)? Slug faz sentido? `status` vai gravar como `draft`?
...
6. Pegue `id`/`slug` do rascunho no fim do log: `gh run view <id> --repo andymodels/refugioFerradura --log`.
