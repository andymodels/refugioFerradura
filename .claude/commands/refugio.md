...
- **Google Maps** (`maps.app.goo.gl`, busca `google.com/maps/search/...`): use **só** para confirmar nome, endereço, telefone e o link do local (pro `servico`). Do Google Maps, o único dado que entra na matéria é o link. **As avaliações/comentários de hóspedes do Google Maps não são fonte de fato nenhum da matéria, nunca** — nem nota, nem estrelas, nem citação direta, nem paráfrase, nem "atribuir" um fato do local a "um hóspede descreveu" / "um dos relatos registra" / "avaliações descrevem". Isso vale mesmo quando o comentário de hóspede é a única fonte disponível para uma informação (ex.: "tem piscina", "tem fogueira") — **se a única fonte de um fato é avaliação/comentário de hóspede, esse fato não entra na matéria**, ponto. Vá atrás da fonte oficial (perfil/bio do Instagram, posts do Instagram, site do local, WhatsApp) para confirmar comodidades e estrutura; se a fonte oficial não confirmar, não afirme.
...
- **Escreva você, com voz própria, os fatos já confirmados** ("A casa tem piscina, lago de pesca e pedalinho" em vez de "um relato descreve piscina, pesca e pedalinho"). Nunca terceirize a descrição do lugar para "avaliações dizem" / "relatos registram" / "um hóspede descreveu, outro contou" — isso não é apuração, é preguiça disfarçada de fonte, e o texto fica robótico e repetitivo. Se um dado só existe em avaliação de hóspede, ele está fora da matéria (ver regra do Google Maps acima), não vira frase atribuída.
- Não repita a mesma estrutura de frase de bloco pra bloco (ex.: sempre abrir com "Um dos X descreve/registra..."). Varie construção, ritmo e ângulo de cada intertítulo como faria um redator de verdade, não um template preenchido.

## Antes de salvar, confira

Título corresponde ao conteúdo? Há ângulo editorial claro? Algum dado foi inventado ou não confirmado? Tem clichê ou repetição? Parece propaganda? Já existe matéria parecida publicada (busca feita)? Slug faz sentido? `status` vai gravar como `draft`? **Releia o `title`, `subtitle`, `excerpt`, `metaDescription` e cada `paragraphHtml` procurando qualquer menção a nota/estrelas/avaliação/comentário/relato de hóspede** (do Google Maps, Instagram ou qualquer fonte) — inclusive frases atribuídas tipo "um hóspede descreveu", "relatos registram", "avaliações dizem". Se achar, reescreva com fato de fonte oficial ou tire do texto, não deixe pra depois. Confira também se cada bloco tem construção de frase diferente do anterior, sem repetir o mesmo esqueleto.
